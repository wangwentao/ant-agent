...
- **MCP 协议支持**：集成 MCP (Model Context Protocol) 协议，支持与外部 MCP 服务交互，扩展工具和服务集成能力
...
1. **记忆系统**：实现会话记忆功能，使 Ant Agent 能够记住之前的对话内容，提供更连贯的交互体验。

2. **知识库检索 (RAG)**：集成知识库检索能力，通过检索增强生成 (RAG) 技术，提升 Ant Agent 的知识储备和回答准确性。

3. **MCP协议完善**：完善MCP协议中Resource、Prompt类型资源的支持，丰富工具和服务能力。
